...
/obj/machinery/blackbox_recorder/proc/save_all_data_to_sql()
	...
	var/DBQuery/query = dbcon.NewQuery("SELECT MAX(round_id) AS round_id FROM feedback.feedback")
	query.Execute()
	while(query.NextRow())
	...
	for(var/datum/feedback_variable/FV in feedback)
		var/sql = "INSERT INTO feedback.feedback VALUES (null, Now(), [round_id], \"[FV.get_variable()]\", [FV.get_value()], \"[FV.get_details()]\")"
