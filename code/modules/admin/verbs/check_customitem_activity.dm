...
/proc/populate_inactive_customitems_list(var/client/C)
	...
	if(ckeys_with_customitems.len)
		var/DBQuery/query_inactive = dbcon.NewQuery("SELECT ckey, lastseen FROM client.player WHERE datediff(Now(), lastseen) > 60")
	...
	if(ckeys_with_customitems.len)
		for(var/cur_ckey in ckeys_with_customitems)
			var/DBQuery/query_inactive = dbcon.NewQuery("SELECT ckey FROM client.player WHERE ckey = '[cur_ckey]'")
			query_inactive.Execute()
