/proc/note_add( var/mob/user, var/note, var/rkey, var/log = 1 )
	...
	var/DBQuery/db_query = dbcon.NewQuery("SELECT ip, computerid FROM client.player WHERE ckey = '[ckey( rkey )]'")
	...
	var/DBQuery/query_insert = dbcon.NewQuery("INSERT INTO staff.player_notes (`id`,`player_ckey`, `player_ip`, `player_cid`, `author_ckey` , `author_ip`, `author_cid`, `author_rank`, `date_time`, `info`) VALUES (null, [sql_rkey], [sql_rip], [sql_rcid], [sql_akey], [sql_aip], [sql_acid], [sql_arank], Now(), [sql_info])")
...
/proc/note_del( var/key as text, var/index as num )
	...
	var/DBQuery/db_query = dbcon.NewQuery("DELETE FROM staff.player_notes WHERE id = '[sql_index]'")
