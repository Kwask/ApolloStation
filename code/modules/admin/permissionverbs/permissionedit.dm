...
/datum/admins/proc/log_admin_rank_modification(var/adm_ckey, var/new_rank)
	...
	if(new_admin)
		...
		var/DBQuery/log_query = dbcon.NewQuery("INSERT INTO admin_log (`id` ,`datetime` ,`adminckey` ,`adminip` ,`log` ) VALUES (NULL , NOW( ) , '[usr.ckey]', '[usr.client.address]', 'Added new admin [adm_ckey] to rank [new_rank]');")
		log_query.Execute()
		...
		if(!isnull(admin_id) && isnum(admin_id))
			...
			insert_query.Execute()
			var/DBQuery/log_query = dbcon.NewQuery("INSERT INTO admin_log (`id` ,`datetime` ,`adminckey` ,`adminip` ,`log` ) VALUES (NULL , NOW( ) , '[usr.ckey]', '[usr.client.address]', 'Edited the rank of [adm_ckey] to [new_rank]');")
			log_query.Execute()
...
/datum/admins/proc/log_admin_permission_modification(var/adm_ckey, var/new_permission)
	...
	if(admin_rights & new_permission) //This admin already has this permission, so we are removing it.
		...
		insert_query.Execute()
		var/DBQuery/log_query = dbcon.NewQuery("INSERT INTO admin_log (`id` ,`datetime` ,`adminckey` ,`adminip` ,`log` ) VALUES (NULL , NOW( ) , '[usr.ckey]', '[usr.client.address]', 'Removed permission [rights2text(new_permission)] (flag = [new_permission]) to admin [adm_ckey]');")
		log_query.Execute()
		usr << "<span class='notice'>Permission removed.</span>"
	else //This admin doesn't have this permission, so we are adding it.
		var/DBQuery/insert_query = dbcon.NewQuery("UPDATE admins SET flags = '[admin_rights | new_permission]' WHERE id = [admin_id]")
		insert_query.Execute()
		var/DBQuery/log_query = dbcon.NewQuery("INSERT INTO admin_log (`id` ,`datetime` ,`adminckey` ,`adminip` ,`log` ) VALUES (NULL , NOW( ) , '[usr.ckey]', '[usr.client.address]', 'Added permission [rights2text(new_permission)] (flag = [new_permission]) to admin [adm_ckey]')")
		log_query.Execute()
