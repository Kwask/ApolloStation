...
datum/admins/proc/DB_ban_record(var/bantype, var/mob/banned_mob, var/duration = -1, var/reason, var/job = "", var/rounds = 0, var/banckey = null)
	...
	var/sql = "INSERT INTO staff.ban (`id`,`bantime`,`serverip`,`bantype`,`reason`,`job`,`duration`,`rounds`,`expiration_time`,`ckey`,`computerid`,`ip`,`a_ckey`,`a_computerid`,`a_ip`,`who`,`adminwho`,`edits`,`unbanned`,`unbanned_datetime`,`unbanned_ckey`,`unbanned_computerid`,`unbanned_ip`) VALUES (null, Now(), '[serverip]', '[bantype_str]', '[reason]', '[job]', [(duration)?"[duration]":"0"], [(rounds)?"[rounds]":"0"], Now() + INTERVAL [(duration>0) ? duration : 0] MINUTE, '[ckey]', '[computerid]', '[ip]', '[a_ckey]', '[a_computerid]', '[a_ip]', '[who]', '[adminwho]', '', null, null, null, null, null)"
...
datum/admins/proc/DB_ban_unban(var/ckey, var/bantype, var/job = "")
	...
	var/sql = "SELECT id FROM staff.ban WHERE ckey = '[ckey]' AND [bantype_sql] AND (unbanned is null OR unbanned = false)"
	if(job)
		sql += " AND job = '[job]'"
...
datum/admins/proc/DB_ban_edit(var/banid = null, var/param = null)
	...
	var/DBQuery/query = dbcon.NewQuery("SELECT ckey, duration, reason FROM staff.ban WHERE id = [banid]")
	query.Execute()
	...
	switch(param)
		if("reason")
			...
			var/DBQuery/update_query = dbcon.NewQuery("UPDATE staff.ban SET reason = '[value]', edits = CONCAT(edits,'- [eckey] changed ban reason from <cite><b>\\\"[reason]\\\"</b></cite> to <cite><b>\\\"[value]\\\"</b></cite><BR>') WHERE id = [banid]")
		...
		if("duration")
			...
			var/DBQuery/update_query = dbcon.NewQuery("UPDATE staff.ban SET duration = [value], edits = CONCAT(edits,'- [eckey] changed ban duration from [duration] to [value]<br>'), expiration_time = DATE_ADD(bantime, INTERVAL [value] MINUTE) WHERE id = [banid]")
...
datum/admins/proc/DB_ban_unban_by_id(var/id)
	...
	var/sql = "SELECT ckey FROM staff.ban WHERE id = [id]"
	...
	var/sql_update = "UPDATE staff.ban SET unbanned = 1, unbanned_datetime = Now(), unbanned_ckey = '[unban_ckey]', unbanned_computerid = '[unban_computerid]', unbanned_ip = '[unban_ip]' WHERE id = [id]"
...
/datum/admins/proc/DB_ban_panel(var/playerckey = null, var/adminckey = null)
	...
	if(adminckey || playerckey)
		...
		var/DBQuery/select_query = dbcon.NewQuery("SELECT id, bantime, bantype, reason, job, duration, expiration_time, ckey, a_ckey, unbanned, unbanned_ckey, unbanned_datetime, edits FROM staff.ban WHERE 1 [playersearch] [adminsearch] ORDER BY bantime DESC")
