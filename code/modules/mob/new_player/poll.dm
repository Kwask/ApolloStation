...
/mob/new_player/proc/handle_player_polling()
	...
	if(dbcon.IsConnected())
		...
		var/DBQuery/select_query = dbcon.NewQuery("SELECT id, question FROM polls.poll_question WHERE [(isadmin ? "" : "adminonly = false AND")] Now() BETWEEN starttime AND endtime")
...
/mob/new_player/proc/poll_player(var/pollid = -1)
	...
	if(dbcon.IsConnected())
		...
		var/DBQuery/select_query = dbcon.NewQuery("SELECT starttime, endtime, question, polltype, multiplechoiceoptions FROM polls.poll_question WHERE id = [pollid]")
		select_query.Execute()
		...
		switch(polltype)
			...
			if("OPTION")
				var/DBQuery/voted_query = dbcon.NewQuery("SELECT optionid FROM polls.poll_vote WHERE pollid = [pollid] AND ckey = '[usr.ckey]'")
				voted_query.Execute()
				...
				var/DBQuery/options_query = dbcon.NewQuery("SELECT id, text FROM polls.poll_option WHERE pollid = [pollid]")
			...
			if("TEXT")
				var/DBQuery/voted_query = dbcon.NewQuery("SELECT replytext FROM polls.poll_textreply WHERE pollid = [pollid] AND ckey = '[usr.ckey]'")
				voted_query.Execute()
			...
			//Polls with a text input
			if("NUMVAL")
				var/DBQuery/voted_query = dbcon.NewQuery("SELECT o.text, v.rating FROM polls.poll_option o, polls.poll_vote v WHERE o.pollid = [pollid] AND v.ckey = '[usr.ckey]' AND o.id = v.optionid")
				voted_query.Execute()
				...
				if(!voted)	//Only make this a form if we have not voted yet
					...
					var/DBQuery/option_query = dbcon.NewQuery("SELECT id, text, minval, maxval, descmin, descmid, descmax FROM polls.poll_option WHERE pollid = [pollid]")
...
/mob/new_player/proc/vote_on_poll(var/pollid = -1, var/optionid = -1, var/multichoice = 0)
	...
	if(dbcon.IsConnected())
		...
		var/DBQuery/select_query = dbcon.NewQuery("SELECT starttime, endtime, question, polltype, multiplechoiceoptions FROM polls.poll_question WHERE id = [pollid] AND Now() BETWEEN starttime AND endtime")
		select_query.Execute()
		...
		var/DBQuery/select_query2 = dbcon.NewQuery("SELECT id FROM polls.poll_option WHERE id = [optionid] AND pollid = [pollid]")
		...
		var/DBQuery/voted_query = dbcon.NewQuery("SELECT id FROM polls.poll_vote WHERE pollid = [pollid] AND ckey = '[usr.ckey]'")
		voted_query.Execute()
		...
		var/DBQuery/insert_query = dbcon.NewQuery("INSERT INTO polls.poll_vote (id ,datetime ,pollid ,optionid ,ckey ,ip ,adminrank) VALUES (null, Now(), [pollid], [optionid], '[usr.ckey]', '[usr.client.address]', '[adminrank]')")
...
/mob/new_player/proc/log_text_poll_reply(var/pollid = -1, var/replytext = "")
	...
	if(dbcon.IsConnected())
		...
		var/DBQuery/select_query = dbcon.NewQuery("SELECT starttime, endtime, question, polltype FROM polls.poll_question WHERE id = [pollid] AND Now() BETWEEN starttime AND endtime")
		select_query.Execute()
		...
		var/DBQuery/voted_query = dbcon.NewQuery("SELECT id FROM polls.poll_textreply WHERE pollid = [pollid] AND ckey = '[usr.ckey]'")
		voted_query.Execute()
		...
		var/DBQuery/insert_query = dbcon.NewQuery("INSERT INTO polls.poll_textreply (id ,datetime ,pollid ,ckey ,ip ,replytext ,adminrank) VALUES (null, Now(), [pollid], '[usr.ckey]', '[usr.client.address]', '[replytext]', '[adminrank]')")
		insert_query.Execute()
...
/mob/new_player/proc/vote_on_numval_poll(var/pollid = -1, var/optionid = -1, var/rating = null)
	...
	if(dbcon.IsConnected())
		...
		var/DBQuery/voted_query = dbcon.NewQuery("SELECT id FROM polls.poll_vote WHERE optionid = [optionid] AND ckey = '[usr.ckey]'")
		voted_query.Execute()
		...
		var/DBQuery/insert_query = dbcon.NewQuery("INSERT INTO polls.poll_vote (id ,datetime ,pollid ,optionid ,ckey ,ip ,adminrank, rating) VALUES (null, Now(), [pollid], [optionid], '[usr.ckey]', '[usr.client.address]', '[adminrank]', [(isnull(rating)) ? "null" : rating])")
		insert_query.Execute()
