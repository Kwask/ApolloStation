proc/sql_poll_players()
	...
	if(!dbcon.IsConnected())
		...
		var/DBQuery/query = dbcon.NewQuery("INSERT INTO stats.population (playercount, time) VALUES ([playercount], '[sqltime]')")
		if(!query.Execute())
...
proc/sql_poll_admins()
	...
	if(!dbcon.IsConnected())
		...
		var/sqltime = time2text(world.realtime, "YYYY-MM-DD hh:mm:ss")
		var/DBQuery/query = dbcon.NewQuery("INSERT INTO stats.population (admincount, time) VALUES ([admincount], '[sqltime]')")
		if(!query.Execute())
			var/err = query.ErrorMsg()
...
proc/sql_commit_feedback()
	...
	if(!dbcon.IsConnected())
		...
		var/DBQuery/max_query = dbcon.NewQuery("SELECT MAX(roundid) AS max_round_id FROM feedback.feedback")
		max_query.Execute()
		...
		for(var/datum/feedback_variable/item in content)
			...
			var/DBQuery/query = dbcon.NewQuery("INSERT INTO feedback.feedback (id, roundid, time, variable, value) VALUES (null, [newroundid], Now(), '[variable]', '[value]')")
			if(!query.Execute())
				var/err = query.ErrorMsg()
