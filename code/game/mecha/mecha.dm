...
/obj/mecha/Topic(href, href_list)
	...
	if(usr.stat > 0)
		return
	var/datum/topic_input/input_filter = new /datum/topic_input(href,href_list)
	if(href_list["select_equip"])
		if(usr != src.occupant)	return
		var/obj/item/mecha_parts/mecha_equipment/equip = input_filter.getObj("select_equip")
		if(equip)
			src.selected = equip
	...
	if(href_list["rfreq"])
		if(usr != src.occupant)	return
		var/new_frequency = (radio.frequency + input_filter.getNum("rfreq"))
	...
	if(href_list["req_access"] && add_req_access)
		...
		output_access_dialog(input_filter.getObj("id_card"),input_filter.getMob("user"))
		return
	if(href_list["maint_access"] && maint_access)
		if(!in_range(src, usr))	return
		var/mob/user = input_filter.getMob("user")
		if(user)
			if(state==0)
				state = 1
			...
			output_maintenance_dialog(input_filter.getObj("id_card"),user)
		return
	...
	if(href_list["remove_passenger"] && state >= 1)
		...
		return
	if(href_list["add_req_access"] && add_req_access && input_filter.getObj("id_card"))
		if(!in_range(src, usr))	return
		operation_req_access += input_filter.getNum("add_req_access")
		output_access_dialog(input_filter.getObj("id_card"),input_filter.getMob("user"))
		return
	if(href_list["del_req_access"] && add_req_access && input_filter.getObj("id_card"))
		if(!in_range(src, usr))	return
		operation_req_access -= input_filter.getNum("del_req_access")
		output_access_dialog(input_filter.getObj("id_card"),input_filter.getMob("user"))
		return
	...
	if(href_list["debug"])
		if(href_list["set_i_dam"])
			setInternalDamage(input_filter.getNum("set_i_dam"))
		if(href_list["clear_i_dam"])
			clearInternalDamage(input_filter.getNum("clear_i_dam"))
		return
	*/
